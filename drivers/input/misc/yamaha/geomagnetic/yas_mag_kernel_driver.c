...
#define SENSOR_NAME			"YAS530B"
#define DRIVER_VERSION			YAS_VERSION
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void geomagnetic_early_suspend(struct early_suspend *h);
static void geomagnetic_late_resume(struct early_suspend *h);
#endif
...
static DEVICE_ATTR(delay, S_IRUGO|S_IWUSR|S_IWGRP|S_IROTH|S_IWOTH,
	geomagnetic_delay_show, geomagnetic_delay_store);
static DEVICE_ATTR(enable, S_IRUGO|S_IWUSR|S_IWGRP|S_IROTH|S_IWOTH,
...
static DEVICE_ATTR(wake, S_IWUSR|S_IWGRP|S_IROTH|S_IWOTH, NULL, geomagnetic_wake_store);
...
#endif /* DEBUG */
static DEVICE_ATTR(info, S_IRUGO, driver_info_show, NULL);
...
static DEVICE_ATTR(offsets, S_IRUGO|S_IWUSR|S_IROTH|S_IWOTH,
...
static DEVICE_ATTR(dynamic_matrix, S_IRUGO|S_IWUSR|S_IROTH|S_IWOTH,
...
{
	struct geomagnetic_data *data =
		container_of(h, struct geomagnetic_data, early_suspend);
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->early_suspend.suspend = geomagnetic_early_suspend;
	data->early_suspend.resume = geomagnetic_late_resume;
