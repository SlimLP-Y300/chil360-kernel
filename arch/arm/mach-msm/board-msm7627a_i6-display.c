...
static unsigned i6_mipi_dsi_gpio[] = {
	GPIO_CFG(GPIO_I6_LCD_BRDG_RESET_N, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA), /* GPIO_I6_LCD_BRDG_RESET_N */
	GPIO_CFG(GPIO_I6_LCD_BACKLIGHT_EN, 0, GPIO_CFG_OUTPUT,
			GPIO_CFG_PULL_UP,
			GPIO_CFG_2MA), /* GPIO_I6_LCD_BACKLIGHT_EN   OUTPUT*/
	GPIO_CFG(GPIO_I6_LCD_BACKLIGHT_EN, 0, GPIO_CFG_INPUT,
			GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA),	/* GPIO_I6_LCD_BACKLIGHT_EN  INPUT*/		
};
...
static int msm_fb_dsi_client_I6_reset(void)
{
	int rc = 0;
      ...
      // printk("linxc: msm_fb_dsi_client_I6_reset \n");

	rc = gpio_request(GPIO_I6_LCD_BRDG_RESET_N, "i6_lcd_brdg_reset_n");
	if (rc < 0) {
		pr_err("failed to request i6 lcd brdg reset_n\n");
		return rc;
	}
...
#if 1	//linxc 2012-12-21  +++
static int i6_dsi_gpio_initialized = 0;
...
static int mipi_dsi_panel_i6_power(int on)
{
	int rc = 0;
	   
	if (!i6_dsi_gpio_initialized) {
		
		printk("linxc: %s  pmapp_disp_backlight_init for backlight \n", __func__);		
		
		pmapp_disp_backlight_init();
		rc = gpio_request(GPIO_I6_LCD_BACKLIGHT_EN, "i6_gpio_bkl_en");
		if (rc < 0){
			pr_err("Failed to request i6_gpio_bkl_en\n");
			return rc;
		}

		i6_dsi_gpio_initialized = 1;
		...
		if (mdp_pdata.cont_splash_enabled) {
			...
			rc = gpio_tlmm_config(i6_mipi_dsi_gpio[2],
					 GPIO_CFG_ENABLE);
			if (rc < 0) {
				pr_err("failed I6 GPIO_BACKLIGHT_EN tlmm config\n");
				goto gpio_fail1;
			}
			rc = gpio_direction_input(GPIO_I6_LCD_BACKLIGHT_EN);
			if (rc < 0) {
				pr_err("failed to enable backlight\n");
				goto gpio_fail1;
			}
			...
			rc = gpio_tlmm_config(i6_mipi_dsi_gpio[0],
			     GPIO_CFG_ENABLE);
			if (rc < 0) {
			...
			rc = gpio_direction_output(GPIO_I6_LCD_BRDG_RESET_N, 1);    //keep reset high from lk
			if (rc < 0) {
				pr_err("Failed GPIO bridge Reset\n");
				goto gpio_fail2;
			}
	...
	if (on) {		
		...
		rc = regulator_enable(gpio_reg_3p3v);		//I6  GPIO12 3.3V for [LCM]
		if (rc < 0)
			pr_err("%s: reg enable failed\n", __func__);
		/*Enable EXT_2.85 and 1.8 regulators*/
		rc = regulator_enable(gpio_reg_2p85v);		//I6  GPIO35 2.8V for [CAM]
		if (rc < 0)
			pr_err("%s: reg enable failed\n", __func__);
		rc = regulator_enable(gpio_reg_1p8v);		//I6  GPIO34 1.8V for [LCD and CAM]
		if (rc < 0)
			pr_err("%s: reg enable failed\n", __func__);

		/*Configure LCD Bridge reset*/
		rc = gpio_tlmm_config(i6_mipi_dsi_gpio[0], GPIO_CFG_ENABLE);
		if (rc < 0) {
			pr_err("Failed to enable LCD Bridge reset enable\n");
			gpio_free(GPIO_I6_LCD_BRDG_RESET_N);			
			return rc;
		}

		rc = gpio_direction_output(GPIO_I6_LCD_BRDG_RESET_N, 1);
		if (rc < 0) {
			pr_err("Failed GPIO bridge Reset\n");
			gpio_free(GPIO_I6_LCD_BRDG_RESET_N);
			return rc;
		...
		gpio_set_value_cansleep(GPIO_I6_LCD_BRDG_RESET_N, 1);		
		msleep(20);
		gpio_set_value_cansleep(GPIO_I6_LCD_BRDG_RESET_N, 0);
		msleep(20);
		gpio_set_value_cansleep(GPIO_I6_LCD_BRDG_RESET_N, 1);
		msleep(20);
...
gpio_fail2:
	gpio_free(GPIO_I6_LCD_BRDG_RESET_N);				
gpio_fail1:
	gpio_free(GPIO_I6_LCD_BACKLIGHT_EN);				
	return rc;	
}
#else
